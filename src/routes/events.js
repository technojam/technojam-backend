...
router.get('/:eventId', async (req, res) => {
	try {
		...
		if (!event) {
			res.status(404).json({ code: 'EVENTS_EVENTID_NOT_FOUND', msg: 'Resource Not Found' });
		} else {
...
router.post('/add', auth, async (req, res) => {
	...
	try {
		...
		if (user.role != 'admin') res.status(401).json({ code: 'EVENTS_ADD_UNAUTHORISED', msg: 'Not authorized' });
		else {
			let eventCreation = await Events.create(event);
			if (eventCreation) {
				return res.status(201).json({ code: 'EVENTS_ADD_DONE', msg: 'Event Added Successfully' });
			} else {
				return res.status(400).json({ code: 'EVENTS_ADD_FAILED', msg: 'Failed: Add Event Operation' });
			}
		}
	} catch (err) {
...
router.put('/register/:eventId/', auth, async (req, res) => {
	...
	try {
		...
		if (registeredUsers.users.includes(userId)) {
			return res.status(400).json({ code: 'EVENTS_REGISTER_REDUNDANT', msg: 'User Already Registered' });
		} else {
			...
			if (userRegistration && addEventToUserProfile) {
				return res.status(201).json({ code: 'EVENTS_REGISTER_DONE', msg: 'Event Registration Completed' });
			} else {
				return res.status(400).json({ code: 'EVENTS_REGISTER_FAILED', msg: 'Event Registration Failed' });
			}
		}
	} catch (err) {
		res.status(500).send({ 'Server Error': err.message });
...
router.delete('/delete/:eventId', auth, async (req, res) => {
	...
	try {
		const user = await User.findOne({ uid: req.user.uid }).select('-password');
		if (user.role != 'admin') res.status(401).json({ code: 'EVENTS_DELETE_UNAUTHORISED', msg: 'Not authorized' });
		else {
			const deleteEvent = await Events.deleteOne({ eid: eventId });
			console.log(deleteEvent);
			if (deleteEvent.n > 0) res.json({ code: 'EVENTS_DELETE_DONE', msg: `Event Deletion Success` });
			else res.json({ code: 'EVENTS_DELETE_FAILED', msg: `Error in deleting` });
		}
	} catch (err) {
		console.log('err:', err);
