...
// @access   Public
router.get('/:evendId/participants', async(req, res)=> {
    try{
        const eventParticipants = await Events.findOne({uid: req.params.eventId});
...
router.put('/:eventId/register', auth, async(req, res)=> {
    const userId = req.user.uid;
    const eventId = req.params.eventId;
    try {
        let registeredUsers = await Events.findOne({ eid: eventId}).select('users');
        if(registeredUsers.users.includes(userId)){
            return res.status(400).json({msg: 'User Already Registered'});
        }else{
            let userRegistration = await Events.updateOne({eid: eventId}, { $push: {users: userId}});
            let addEventToUserProfile = await User.updateOne({uid: userId}, { $push: {registeredEvents: eventId}});
